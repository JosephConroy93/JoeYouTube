...
- **`script-writer` Prose list + self-check 7, Voice rubric clause in all three formats, `voice.english` in `series.md`/conventions, `tools/script-lint/lint_script.py` at WORKFLOW Step 4.** A lint of the last three scripts found the machine-writing tells creeping in (52 em dashes and seven reveal-flips in Pharaoh's Servant, one in Embalmer, "honor" and "toward" in a British narrator's mouth) with nothing in the agent guarding them; the register rules stay as they are, the list only names the surface habits, and the lint counts them so the scorer quotes lines rather than impressions. British English is a series value, not an agent rule, because the narrator is.
- **`generate-voiceover` `tts.ps1 -ChapterGap` / `voice.chapter_gap`; `prerender.py` chapter card holds 2 s still, then grows 1.8 s (was a 2.2 s grow).** With a gap set, each chapter is its own voice segment and its WAV ends on that much silence, so the timeline has a breath before every card; the card stays readable over the spoken callout before it moves. Why: on Cromwell the cards landed straight after the last word and moved at once, too quick to read (operator). Measured on a scratch run: +1.2 s of silence at -91 dB on every segment but the last, loudness unchanged; a test card is still to 0.03 mean pixel difference across its first 2 s and lands on the scene frame.
- **Text cards are illegible handwriting in the image, no drawn word** (`scene-prompter`, `write-prompts`, `validate-scenes`, `conventions.md`, `place-scenes` 5c now legacy, `check-manifest.py` warns on a text-card prompt without "illegible"). Why: the drawn overlay words on Cromwell floated flat over the page and read as janky (operator); the narration carries the words. QC passes legible words that fit the scene and fails only wrong ones (operator: a rendered "contract" on a contract is fine).
- **`script-writer` Prose list: the nine-level variant's per-level aphorism is a dry observation from the rung, never a coined epigram.** Why: today's prose rule banned aphorism closers and would have contradicted the format.
- **Format modules own the naming: `rank-ladder.md` and `life-story.md` gain a Naming table (`chapter.unit/heading/file/spoken`); `tts.ps1`, `check-manifest.py` and `scene-prompter` read `video.md`, then the format, then `series.md`.** Rank videos are spoken `Level N. <Rank>.`, narrative videos `Chapter N. <Name>.` cards, both on the cream chapter card. Why: operator, every rank video uses levels and the narrative ones chapters; the series default was chapters, so a rank video without its own override would have lost its callouts. Tested with dry runs: rank-ladder voices "Level 1. One.", life-story drops the heading.
- **`tts.ps1` strips HTML comments before segmenting.** Why: East India Company's script opens with two `<!-- -->` pipeline notes above Level 1; they became a 201-character segment 1 that came back as 1.3 s of near-silence (caught on the one-segment check, archived).
- **`merge-floor.py` also splits over-ceiling rows** at the sentence boundary nearest the middle when both halves reach the floor (hook rows excepted; the second half's beat is marked "(cont.)"). Why: East India Company's beat sheet came back with ten rows over the 11 s ceiling and `check-manifest.py` said "split it" with no tool to do it; eight split automatically, two (a short lead-in before a long quotation, a single long sentence) needed a hand split that the merge pass then folded into the previous row.
- **`get-scenes`: a `code 13 Internal error` result is the server's fault, not a refused prompt** — resubmit unchanged. Why: East India Company's Level 1 pilot came back after ~6 h with 11/11 results carrying that error and no image; the skill's existing wording would have sent eleven good prompts to be rewritten. The model id was checked against the API's model list first (still live, still batch-capable), then the chapter was resubmitted.
- **`content/prompt-hardening-rules.md` R14, R15, R16** (channel file, untracked): never name a flag's design (a "red-and-white striped flag" renders as the Stars and Stripes); in a blank-head style name every exposed body part white, not just heads and hands, or bare-armed background figures drift to naturalistic skin (three occurrences across Levels 1, 4 and 5); never name a document by a word that could be printed on it ("a folded order" letters ORDER onto the prop, once mirrored), and "illegible handwriting" alone can come back as Lorem ipsum.
- **`batch-log.md` has one writer at a time** (`validate-scenes`, `generate-scenes`). Why: two fix batches submitted while a QC agent was rewriting the log lost their rows under its rewrite, so nothing tracked them and their images were never fetched; they were recovered with `get-scenes -BatchId`. QC a chapter, then submit the next.
- **`check_render.py`: an SFX row with `until` is measured across its whole held span.** Why: East India Company's monsoon rain runs from 077 into 078, so the check's quiet second landed inside the sound and the row read BAD on a correct render; it passes once the span ends at the `until` scene.
- **Narration numbers and dates are written as spoken words** (`script-writer` Prose list, `tools/script-lint` digit check, WORKFLOW Step 4). Why: East India Company's "£1,380,269" and bare dates came out of ElevenLabs as noise; the operator could not make out 12:48-12:55 at all. A figure too long to say is rounded in-voice with the exact number left in the research file. `tts.ps1` speaks a level heading's digit as a word, so the card keeps `Level 6.` while the voice says "Level six."
- **`prerender.py --tail --outro` and `build_timeline.py --tail`**: the last scene holds past the last word and a cream outro card (series name, thank-you, 0.6 s fade-up) follows it. Why: the operator found the ending stopped dead about 0.2 s after the last word.
- **`rank-ladder.md` Ending rewritten to three beats and `script-writer` allows one reflective closing sentence under 20 words**; also contractions by default, one soft-quantity hedge per paragraph, no fact standing alone for more than a sentence, second person held to the end, CTA at a third of the way in. Why: a study of ten POVrank transcripts (`research/artifacts/povrank-voice-study.md`) found their endings run callback, reflection, cold final image, and our own "no spoken moral" rule forbade the middle beat.
...
- **`tools/script-lint` prints the register bands** (mean sentence length 8-11 words, contractions never zero, second-person 5-8 per 100 words, fact-only sentences under 20%) and lists the fact-only sentences. Why: a 20-video POVrank corpus (`research/artifacts/povrank-register-targets.md`) turned the register into numbers, after "dry" drifted into clinical. East India Company's script reads 12.4 words, 1.1 contractions, 3.8 second-person: long, and further from the reader than the channel it competes with.
- **`check_render.py`: a Static row passes above SSIM 0.982, not 0.99.** Why: x264 noise on a flat still lands about 0.985, so two text cards failed on both East India Company renders while their source clips measured still to 0.03 of a pixel level.
- **`captions.py --max-lines`**: `1` puts a single line on screen at a time, the way the genre's channels caption. Why: the operator wants captions burned into the picture, not only uploaded to YouTube, and found two lines at once too much to read. Burn-in settings that worked on East India Company at 1080p: Arial Black, FontSize 17, Outline 2, Alignment 2, MarginV 16 (libass scales from a 288-line reference, so a FontSize of 26 rendered about 97 px tall and sat a third of the way up the frame).
- **WORKFLOW Step 11: tags are capped at 500 characters in total**, not per tag. Why: East India Company's tag list was written at 792 characters and the operator had to trim the low-scoring ones in Studio; YouTube drops the overflow silently.
...
- **Step 8 prompts are written by the driving session through the new `write-prompts` skill** (recipe: shot, `[[ID]]`, action and emotion, one dressed setting clause repeated across a run, light; one or two figures; no words in the image; shot-spread targets from the census, counted by `check-manifest.py`; reference rule: main characters attach their reference, nothing else gets one). Why: the reproduction that worked was hand-written, and the operator wants the recipe and spread kept where they cannot drift.
- **`reference-heads.py`** (generate-scenes/scripts) and a Step 6 line: check every figure reference's head crop before use. Why: the Embalmer wab reference carried a drawn nose that the sign-off sheet missed at full-figure scale; all 18 Level 3 scenes inherited it and the level was regenerated (70p); the lector reference had a nose and ears too.
- **rank-ladder format: the hook question lands in the first 8 seconds after the callout.** Why: Embalmer's "for the head — whose?" arrived 19–32 s in, after 19 s of setup; operator found it late.
- **Style leniency: hair, beards and headwear that suit a role are notes, not fails; hook shots are medium or close, never wide.** Why: operator on Embalmer — the old master's grey beard (147) would have been fine, and the hook's one wide shot (002, the yard) sells a place instead of a face and was also the clip that failed.
- **generate-hook motion prompts: no heat shimmer, haze or glow; name what stays still.** Why: Embalmer hook shot 2's "heat shimmer over the far wall" turned the sky into a wall of flame by mid-clip.
- **generate-hook `trim-hook.py`**: each raw clip cut to its scene's span (start to next start), audio stripped, last frame held when short, frames measured. Why: the skill described the cut but nothing performed it; done by hand on Embalmer, now a script.
- **veo.ps1: retry on 429, log operation names, skip downloaded shots.** Why: the first Embalmer hook run hit Veo's per-minute quota on shot 4 and exited after submitting shots 1-3 without recording their operations; they were recovered by listing the model's operations and matching first frames to the stills.
- **align.py reads unbackticked scene ids and an fps cell with a note; finalize-scenes gains `promote-latest.py`** (latest attempt becomes the canonical file, the old bare file kept as an attempt; dry run by default). Why: the lightweight manifest writes ids without backticks, which aborted the row count; Embalmer had 54 scenes whose winning image was an attempt file.
- **write-prompts: age by posture and props only, with the blank head stated.** Why: "stooped with age" gave Embalmer's old master forehead creases across Level 9 and a full grey beard in 147.
- **write-prompts: name a document by what it is, not what it says.** Why: "a rolled papyrus contract" came back with CONTRACT written on it in English (Embalmer 128).
- **Edit: spot SFX only, no ambience beds; transitions off by default** (`video.md` `sfx` and `transitions` keys; WORKFLOW Step 9, plan-ken-burns). Why: operator found the Pharaoh's Servant ambience overlays didn't suit; wants a small library of on-screen action sounds built up by use; transitions wait until the script-driven edit is proven on the locked Resolve version.
- **write-prompts: callback lines may reuse an earlier validated still** (`reuse: NNN`, file copied, row not submitted). Why: Embalmer Level 7's flashback lines point at scenes already on screen; a new render is weaker and costs a submission.
- **write-prompts: close shots on objects keep the figure attached** ("seen from the chest up, holding X"). Why: Embalmer 096 drew foreground hands belonging to nobody beside a full figure, and 085 v1 drew the protagonist twice, both from "the hands of X … his head soft behind".
- **write-prompts: no tool touching a body; get-scenes: a no-image result is a refused prompt, rewrite it.** Why: Embalmer 100 (a bronze spatula at the head of a covered body) came back with no image part.
- **validate-scenes calibration**: face rules apply to figures, not masks, statues or carvings; a figure with no face fails; white skin blending into a white kilt is not a costume change. Why: Level 4 QC failed a funeral mask for having a nose, missed nothing on a faceless father, and twice read the father's bare chest as a robe.
- **get-scenes: bounded background wait** (status every 2 min, 40 min cap, then fetch) replaces "never poll". Why: operator asked the session to wait on batches itself instead of being nudged; a step toward the unattended per-level loop.
- **validate-scenes style check: a faint nose curve is a note, not a regenerate** (fully drawn faces and tan skin still fail; references stay strict); a short neck is a note too, since the source frames draw one on standing figures. Why: operator call on Embalmer Level 3; QC then flagged necks on figures matching approved chapter 1 and the StreetFighter source frame; hardening rule V9.
- **`merge-floor.py`** (generate-scenes/scripts): sub-floor beat rows join a neighbour under the ceiling, ids renumber, index ranges follow; WORKFLOW Step 7 calls it when the floor band is high. **write-prompts**: joined beats take the image for the last line; extras keep the `[[EXTRA]]` token with role detail in words; a re-prompted hook still rewrites its plan row. **CLAUDE.md** working rule: a chat fix is written into the file that should have prevented it before the next level. Why: the Embalmer beat sheet came back 35% sub-floor and was merged in the scratchpad; the operator asked that nothing operational live only in chat.
- **Extras attach a stock extra reference per style** (`content/styles/extras/<Style>-Villager.jpg`, `[[EXTRA]]` cast line). Why: chapter 1 in the recipe passed 29/30 at viewer level; the one fail was again an unreferenced extra rendered tan, the fourth time in this video, while every referenced figure held.
- **Step 6 renders one reference per main character** from its cast line (extras, settings, objects never). Why: recipe test, references 6/6 vs text-only 4/6 on staging, style equal.
...
- **place-scenes scripts** (`prerender.py`, `sfx.py`, `build_timeline.py --cards --sfx`): one run pre-renders stills, swaps hook clips 1:1 onto their scenes, draws overlay words and level cards; `sfx.py` bakes `claude/sfx-plan.md` (new schema in conventions) to measured dual-mono clips that go into the XML with the voice. Why: the Embalmer edit had these as prose steps with "add a --cards option before the first edit", and the hook-as-prefix builder no longer matched the 1:1 hook plan.
- **XMEML audio imports as mono centre-panned tracks** (place-scenes audio traps): every audio clip now carries a +3 dB Audio Levels filter in the XML. Why: the first Embalmer range renders measured exactly 3.0 dB under the voice file with identical L/R RMS; readback showed mono tracks. Also: `import_timeline_checked`'s sanitizer reads Windows `file://localhost/C:/` paths as missing and imports an empty timeline, so place-scenes imports with it off.
- **apply-fusion batch covers Focal and Pan** (`plan_to_spec.py`, `apply_baseline.lua`), with `report_kb.lua` + `check_kb.py` readback and `check_render.py` range measurement (frames, LUFS and per-channel RMS, SSIM motion, card luma, SFX as the render-minus-voice residual). Why: the untested Lua wrote spline handles as absolute points, which Fusion reads as offsets (Size 1.87 on a 1.15 zoom, caught by a render); pan direction measured (camera right = Center.x falling). WORKFLOW Step 9: range renders first, full render on the operator's go. `check_render.py` also fails an audio-only render: after an audio-only job, the next `set_settings` left ExportVideo off twice while `verify_output` reported verified.
- **Edit retro** (`plan-ken-burns`, `apply-fusion`, `generate-hook`): `draft_plan.py` applies the per-scene motion rules to the subagent-confirmed picks (reproduces Embalmer's plan row for row) and a chapter's last scene releases on Out+EO; the plan's `dur` comes from the place-scenes frame plan and `Pan` points are documented as Transform Center positions; apply-fusion's order of work, description and verification now match the batch-plus-readback path that ran (window capture optional, Resolve launch fallback, current-timeline note); generate-hook no longer says the card is a prefix. `check_render.py` fits SFX lag and gain on the louder neighbouring second against a frame-snapped voice reference, and gains `--skip-motion`. Why: the plan was drafted from a scratchpad script, the docs still described the per-scene path and a capture step that never ran, and scene 030's SFX check passed on a near-silent second 12 ms off at a segment join.
- **WORKFLOW Step 11**: the channel name comes from `series.md`, not a hard-coded "Watcher POV". Why: the channel is renamed Lived It (operator, 2026-09-15).
- **Step 10 is the cream-card thumbnail, built by the new `make-thumbnail` skill** (`scripts/thumb.py`): series cream ground, 2–3 hook words stacked left in the series font with one accent line, one scene still as a tilted outlined card right, a 1280/360/168 px check sheet; colours and font are new `series.md` `thumbnail.*` keys, the chosen pair goes in `video.md` `thumbnail`. `--scene` takes canonical stills only; `--still` names an attempt on purpose. Why: operator chose this look for Embalmer (Test & Compare: "WHO WORE THIS?" v "NOT THE HEAD.") and wants it on every video; no generation keeps the text exact and the layout locked; the draft script took `109_…attempt-1.jpg` because it sorts before the canonical file.
- **Step 6 costume lookup**: a Sonnet subagent, five-minute cap, returns a costume per role and at most five era don'ts for the cast sheet. Why: the cast sheet read "the research file's visual notes", which only existed on Embalmer as a leftover of the old flow; Step 3 stays about narration.
- **Stock extra moves to Step 0** (rendered once per style). Why: it is a style asset, not a per-video step.
- **Stale old-flow wording removed**: `chain-scenes` mentions (generate-scenes, get-scenes) and its empty folder; conventions `protagonist`, `reference-images` and `visual_guardrails` rows; Eggline "text locks, not images"; series `chapter.heading` now `Level N. The <Role>.` to match the scripts. Why: a fresh or unattended run follows the docs literally.
...
- **`life-story` format** (new module): one documented person's life told to them in the second person, 8–10 named chapters, legends as stories in-scene, own score table. Why: operator pivot on the Cromwell concept, a real life told in chapters instead of a composite rank ladder.
- **Chapters replace levels as the series default** (`series.md` `chapter.*` → `Chapter N. <Name>.`, `chapter-NN.md`) with a new `chapter.spoken` key: `no` puts the heading on the card only, so `tts.ps1` skips it, `check-manifest.py` leaves it out of the bookmark span, and `scene-prompter` starts each chapter at its first spoken words; `prerender.py` cards read any `## ` heading. Embalmer's `video.md` pins the old level keys. Step 8 docs say chapter, not level. Why: operator wants chapter names for flexible narration instead of forced level callouts.
- **Hooks are built for animation** (script-writer, scene-prompter, write-prompts, generate-hook; `life-story` allows a 15–20 s prologue flash-forward and lands the hook within 20 s): the opening lines are physical action a still can freeze mid-motion, and the motion prompt finishes it. Why: operator, the Embalmer hook (a boy washing dishes and walking) animated as nothing happening.
- **`tts.ps1` audition fixes**: the voice override is `$AuditionVoice` (alias `-VoiceId`) because PowerShell variable names ignore case and `$voiceId` was overwriting it (two takes came out as the default voice); `eleven_v3` requests drop `previous_text`/`next_text`, which that model rejects. Why: found on the first real audition.
...
- **Run knowledge from the Cromwell test moved out of chat** (WORKFLOW Steps 3, 6, 8; prompt-writer; scene-prompter; validate-scenes): parallel chapter waves after the pilot with the driving session owning the index; the gathering-agents-then-Opus research pattern; a "figures written inline" list in the cast sheet; `## Prologue.` as `chapter-00.md`; what a QC brief contains and a look at any wholesale style fail before resubmitting. Why: so a fresh session can run a video from scratch without this conversation.
- **WORKFLOW Step 8 end names `promote-latest.py --apply`, `align-scenes --source api` and the Veo quota; the hook plan belongs to the first chapter (the Prologue when there is one)**. Why: a read-through as a fresh agent found the promote step and the prologue case missing.
...
- **Gemini pricing verified and lite tested**: batch flash 2K ≈ £0.039, 1K ≈ £0.026, lite ≈ £0.013 (1K only, 2K rejected). Lite passes on medium/close sparse-background scenes in flat styles, fails on dense wides; allowed per row for those beats only (`generate-scenes`).
- **Step 8 is now a per-chapter loop** (prompts → submit → fetch → validate → harden → fix → next chapter's prompts), so each chapter's failures become the next chapter's rules; Step 7 is bible and style once; `align-scenes` moves to after the last chapter. `scene-prompter` re-reads hardening entries added since the previous chapter. **Model routing table** in CLAUDE.md (Opus for judgment, Sonnet for lookups and checklist QC, Haiku for mechanics); both agents pinned to Opus; `validate-scenes` dispatches on Sonnet.
- **Hardening log split**: `content/prompt-hardening-rules.md` (promoted rules, validation calibration and watch list, one line each) is what prompt writers read every chapter; `prompt-hardening-log.md` becomes the incident archive read on demand. `scene-prompter` now reads research sections (fact-check, overstatement, visual notes and addendum, the chapter's rungs), checks the style by heading, and never reads other videos' bibles. Why: the per-chapter loop re-reads inputs nine times a video; ~42k words of reads dropped to roughly 17k.
- **Voiceover files named `<slug>_voice_NN`** (mp3, normalised wav, segment txt, alignment json) so every video's audio is searchable; speed comes from `voice.speed` in config, test takes use `--tag`. **Section-scoped reads**: script-writer reads only its concept/sources sections and the named register sections; scene-prompter Mode 2 reads only the chapter's script sections and bible entries (bible gains an index table); plan-ken-burns extracts manifest columns; validate-scenes greps batch-log rows; channel-farmer and style-farmer grep their index sections; CLAUDE.md lists the grep-don't-read files. `research/index.md` split into current state plus `index-history.md`. Why: per-chapter loops multiply every whole-file read.
- **Style drift fix (Eggline)**: channel-farmer now measures proportions and skin on frames with bare limbs before writing a style, judges test renders against source frames side by side, and needs operator approval before an entry is usable; style entries gain `Source frames:` and `Identity:` lines. validate-scenes gains check 5 (style holds, judged against source frames). scene-prompter Mode 1 describes figures by silhouette, costume and props only. WORKFLOW Step 7 renders three references for operator sign-off first; any style deviation is a regenerate. Hardening rules R13 and V8. Why: the Embalmer references came out with tan naturalistic bodies and small oval heads, and every check passed them because each compared against the previous mistake.
- **Visual policy: scenes look good first.** WORKFLOW Step 6 becomes a visual dressing pass (look to aim for plus immersion-breakers only; accuracy nit-picks are not guardrails); scene-prompter treats a bare or monochrome setting as a failed prompt. Narration accuracy is unchanged. **Eggline**: head-tint rule replaced (heads and hands stay white; light only shades them) after it turned torch-lit heads solid orange; a global white-skin rule was tried and reverted after it stripped clothing in the approved cellar scene, so the skin instruction lives in figure prompts. Why: the operator found Egypt settings drab and the references off-style.
- **Hook wired into the workflow**: scene-prompter marks chapter 1's hook shots and writes `hook-plan.md`; `generate-hook` runs once those stills validate, timed from the ElevenLabs alignment; `place-scenes` swaps each hook still for its clip 1:1 and adds 2 s level cards (nine-level variant no longer uses Gemini title text-cards). Why: the skill existed but nothing invoked it, and place-scenes still assumed a separate hook script.
...
- **Prompt blocks**: locked subjects are `[[ID]]` tokens expanded at submit from `claude/prompt-blocks.md` (noun + `{ref}` ordinal + locked description, guards joined into one sentence, `_closing` line); `gemini-batch.ps1 -Action expand` prints the sent text; scene-prompter Mode 1 writes blocks, Mode 2 reads blocks instead of whole bible entries and writes no guard or closing sentences, Mode 3 edits a block instead of sweeping rows. Why: Embalmer Levels 1–2 took 36 min for 30 rows; ~400-word prompts for ~19-word beats, most of it the same descriptions retyped. Round trip on the real rows: 29/30 expand identically bar the standardised guard sentence, 48% fewer words to write.
- **`check-manifest.py`** (new, generate-scenes): word counts and bands, ceiling, bookmarks verbatim and uncovered narration, numbering and index ranges, style cells, reference cells and binding, tokens, text-card strings, mascot, hook plan; FAIL blocks submission. scene-prompter no longer counts words or reports statistics. Why: the agent has no shell and was counting and cross-checking by hand; the script reproduces its Level 1–2 numbers exactly and caught all five planted errors.
- **`gemini-batch.ps1 -RefMaxPx`** shrinks references in memory before inlining. Why: 2K references (1.6–2.6 MB) split 30 scenes into 18 jobs; at 1376 px a four-reference row drops from 10 MB to under 1 MB. Google documents a fixed per-image token budget, so size does not change cost; identity fidelity at 1K is still to be tested before it becomes the default.
- **`_closing` appended sentence by sentence** (a prompt that already holds one closing sentence gains only the missing ones). Why: Embalmer chapter 1 QC failed 4 of 30 on Eggline face/skin drift; the fix adds a face sentence to `_closing`, and the legacy rows being resubmitted already carry the first sentence.
- **Scene ceiling 9 s → 11 s** (scene-prompter, check-manifest). Why: the Street Fighter shot census (244 cuts) holds a median 6.6 s with 22% of holds over 9 s; operator call 2026-09-14. Fewer scenes to prompt per video.
- **`-RefMaxPx` defaults to 1376.** Why: blind six-scene test (`research/artifacts/reftest-2026-09-13`) scored 1K references equal to full-size on identity (27/27) with job bodies ~10× smaller; lite with references scored lower (24/27) and stays limited to simple shots.
- **generate-scenes docs**: lite is no longer described as taking no references (Google lists up to 14; untested here); reference cost stated as ~£0.0002 per request.
...
- `rank-ladder.md`: weight the split animation budget by the retention gradient rather than spreading it evenly — two clips on level 2 (the cliff, and the cut between them reads as a restart), one on level 3, none from level 4 on. Only the clip under a card needs the ~8 s beat. Why: a clip late in the ladder buys insurance on viewers who have already decided to stay.
- WORKFLOW Step 6 + prompt-hardening R17: every cast line must state hair, facial hair and headwear; "bald" is a per-figure choice, not a default. Why: the Eggline entry has always allowed hair and calls it the strongest single identity marker, but every East India Company cast line read "bald blank egg head", so a whole video shipped with no hair on anyone. Two figures who must not be confused never share a hair shape and colour.
- `script-writer`: everything in script.md that is not narration must be an HTML comment. Why: the Krays draft carried its format note as plain text, and the voiceover dry run showed segment one would have opened the video with "Format: rank-ladder nine-level. Second person, present tense, British." East India Company only escaped it because its preamble happened to be commented.
- `generate-voiceover` (`tts.ps1`, SKILL.md): `--dry-run` no longer writes `claude/voiceover-segments/*.txt`. Why: those files are the record of what was actually voiced and are diffed against the script to decide which segments need regenerating after an edit — but a dry run silently overwrote all nine, so the check destroyed the evidence it exists to protect. Caught while confirming only segment 1 of the Krays script had changed.
- `script-lint`: an **ear checks** block — trailing which-clauses, you-clauses appended to a third-person sentence, referents held across a sentence, number-fragment runs, abstract density — quoting whole sentences. Why: the Krays script passed every band and every prose tell and still carried thirteen lines that fell over when spoken; the bands were being met by the cheapest route (a reversal folded into a trailing clause, a "you" bolted on to a fact), and nothing in the chain measured that.
- `script-writer`: Prose gains three rules (second-person density earned inside the fact, never appended; a reversal keeps its own sentence; a fact lands where it can be attached), self-check item 8, a Mode 2 note that a pass which moved a band is where ear faults are introduced, and **Mode 4 — READ ALOUD**: an eight-point checklist judged at the narrator's pace, tiered 1-3, run after the lint and before Mode 3. Why: the lint measures the page and Mode 3 scores structure; neither hears, and the operator was the only thing in the pipeline listening.
- WORKFLOW Step 4: lint → Mode 4 → Mode 3, and re-run both after any Mode 2 pass that moved a band. Step 5: segment 1 first, the operator listens, then `-All`.
- `generate-voiceover` (`tts.ps1`): a first run on a video generates **segment 1 only** and stops; `-All` generates every segment without audio; a plain run on a voiced video refuses with guidance. Why: Step 5 already said "listen to one segment before generating the rest" and it was skipped, because a rule in prose is not a gate. SKILL.md also records that `eleven_multilingual_v2` has no per-word emphasis and that stress is placed by sentence position.
- `script-writer`: a word that must carry stress ends a short sentence. Why: `eleven_multilingual_v2` has no per-word emphasis (tested 2026-09-16: capitals did nothing to Jim; "The number was never yours" carried the stress that "It isn't your number" could not), so stress has to be written into sentence position at write time, not asked for at the voice step.
- `style-test.ps1`: a candidate with no image parts is reported with its `finishReason` and skipped, and the response is saved, instead of crashing under StrictMode and abandoning every prompt after it. Why: the Krays reference run died on prompt 4 of 6 with "property parts cannot be found" and never reached its own no-image fallback, so the reason (a named real person, `IMAGE_OTHER`, now rule R18) had to be found with a separate probe.
- `generate-scenes/scripts/edit-image.py` (new) and WORKFLOW Step 6: a figure that must be identical to a rendered reference except for one marker is made by editing that reference, never by a second render. Why: the Krays twins have to match to the pixel; Ronnie is Reggie's file with horn-rimmed glasses added, verified on the head-crop sheet with nothing else moved.
- `generate-voiceover` SKILL.md: the `eleven_v3` delivery controls from its prompting guide (capitals for emphasis, ellipses for pauses, no SSML breaks, no speed parameter, bracketed audio tags of stated inconsistency, three stability modes). Why: the operator plans to trial v3 at its natural pace on the next rank-ladder video because 1.07 on v2 sounds fast on listen-back, and the audition should start from what the model documents rather than another search.
- `check-manifest.py`: HTML comments are stripped from the script before bookmarks are matched. Why: the voice step never speaks a comment, so a bookmark legitimately spans one; the Krays script carries its shot markers as comments after each level callout, and three level-opening bookmarks failed verbatim matching on text nobody will hear.
- WORKFLOW Step 8.5: a second failure that is a removal or positioning fault (lettering, a stray prop, a misplaced figure) on an otherwise passing image is fixed by one `edit-image.py` pass on the best attempt, not a third generation. Why: Krays 038 came back with garbled wall signs twice despite the clause; one edit blanked the boards and moved nothing else. The Pharaoh's Servant mascot cameo taught the same thing by hand.
- `generate-hook` SKILL.md and WORKFLOW Step 8: the hook's Veo submissions start as soon as every still the plan names is validated and canonical; only the trim waits for `align-scenes`. Why: Veo lite allows about four submissions a day, so a seven-shot hook spans the 08:00 reset, and starting it only after the last chapter closes costs a calendar day on a two-to-three-day cadence; the hook's stills sit in the first chapters, which close first.
- `generate-hook` SKILL.md, motion-prompt discipline: a figure told to stay somewhere drifts unless given an action that completes there; lock the camera off when every figure must hold. Why: the Krays' shot 3 dissolved its figures on a tracking prompt and walked the photographer back on a "stays" prompt; only a completing action at his spot (the flash firing) with a locked-off camera held him, at the cost of two extra clips.
- `generate-hook` SKILL.md: a card-landing clip is trimmed to its full beat like any other; `place-scenes` holds its first frame for the card and bounds the output at the beat, so only the clip's first (beat minus card) seconds play. Why: an open note said trim-hook must subtract the card hold; reading `prerender.py hook_job` showed the subtraction would change nothing and the real constraint is on the motion prompt, which must finish inside that window.
- `generate-voiceover` SKILL.md: a re-split that lands on fewer segments leaves a stale highest-numbered segment .txt with no audio; archive it at the time. Why: the Krays script was re-split from ten segments to nine, and the orphan `_voice_10.txt` (byte-identical to `_voice_09.txt`) sat unnoticed through voicing, alignment and the whole scene pass before aborting `captions.py` at publish — align-scenes ignores such a file, so nothing else catches it.
- `script-writer`: new Prose rule "A name earns its definition" and Mode 4 check 9 (definition before consequence). Why: an external script review scored East India 8.7 and named one fix — the dastak is defined in full before the script says what it is worth, so the viewer absorbs exposition passively; the same review of the Krays put its only drop-off trigger at the Tower sentence, where two men are named before there is a reason to hold the names. One rule covers both: say what it does to someone, then name it.
- WORKFLOW Step 4.6 (external script review, before the voice step), `script-writer` Mode 2 review contract, new Prose rule (a term outside common English is glossed at first use) and Mode 4 check 10; `rank-ladder`: a level's first beat is a problem not a description, and a motif changes its angle on each return. Why: the external reviewer's analysis is free (only its rewrite is paid, and Mode 2 does that), but both reviews so far were read after voicing, when a fix costs credits and a re-voiced segment; the run's own findings were exposition-before-stakes, six unglossed period terms taking wide appeal to 6/10, and structure at 7/10 for levels that open on description.
- `lint_script.py`: two ear checks — a term named after its definition, and phrases returning verbatim (motif fatigue). Why: an external review put East India's wide appeal at 6/10 and repetition at 7/10; both faults are mechanical, so the linter should find them without spending a scarce review. Validated against that review — the definition check quotes exactly its Risk #1 sentence, and the repetition check separates East India (9 repeated six-word phrases) from the Krays (1).
- WORKFLOW Step 4.6 rewritten: the reviewer's free score runs on every script before voicing; its deep report is scarce, never spent on known faults, and only on a script written wholly under the current rules. Why: the operator confirmed the score is unlimited and free while deep reports are not.
- WORKFLOW Step 11 3b: the caption burn-in starts as soon as the master's measurements pass, not after the operator's watch-through. Why: burned-in captions were already the standard, but the step did not say when to run it, so it got deferred behind the watch and the operator had to ask whether it had been forgotten; it is background CPU and a re-cut costs only the same encode again.
- `rank-ladder` sets `voice.chapter_gap: 0`; `conventions.md` states the rule: a gap belongs only to an unspoken heading, and a non-zero one is set as a whole number of frames at the video's fps. Why: the gap exists so a viewer can read a card that nothing speaks, but on a rank-ladder the callout is spoken and its own sentence pause is the breath — the operator heard the extra 1.2 s as a delay at every chapter. It was also mis-set at 1.2 s (28.8 frames at 24 fps), so removing it re-rounded every later scene by a frame and forced a full pre-render rather than eight clips.
- `prompt-hardening-rules.md` R21: never say a figure's arms or body are the same blank white as its head — name the garment instead. Why: the clause was meant to stop skin tones on Esmeralda's patrons and instead produced bare white torsos with no dinner jackets, several of them bald and faceless; QC passed both because it checks heads, not clothing, and the operator found them on the watch-through.
- `generate-hook/scripts/check-hook.py` (new) and SKILL.md step 3: hard QC on every hook clip — samples the staged clip every 0.25 s and fails on a hang (0.5 s+ of unchanging picture) or a glitch (a step over 5x the clip's own median). Why: the operator watched a 2.5 s freeze at 1:20 that no check would have caught; it was `trim-hook` holding the last frame of a 4 s clip across a 6.6 s beat. Validated against that exact master — it reports 014 hanging 2.50 s from 4.00 s, finds a second 0.5 s hang on shot 2 that nobody had noticed, and passes the three card-landing shots whose leading static is the card by design.
- `place-scenes` SKILL.md 6b: after any media file is replaced on disk, import into a fresh Resolve project rather than the existing one. Why: the Krays' voice WAVs were re-normalised and padded under a project that already held them; the v1 timeline then rendered silence (-70 LUFS) and the v2 timeline rendered 18 dB hot (+2.0 LUFS, true peak +6.3 dBFS) from an XML byte-identical in its level filters. The same XML in a clean project rendered -14.6 LUFS, true peak 0.0. `media_pool.refresh()` did not fix it.
- `place-scenes/scripts/sfx.py`: every baked SFX is capped at a -8 dBFS true peak, by clean attenuation rather than limiting, and the report prints TP alongside LUFS. Why: raising the Krays' spot sounds so the operator could hear them pushed the master to +2.1 dBFS true peak with both channels pinned at 0.0 — clipping — because place-scenes adds +5 dB to every audio clip in the XML on top of the baked level. A limiter was tried first and was wrong: a short impact carries nearly all its energy in the transient, so limiting to the ceiling dragged its gated loudness 5-8 dB below target and pumped the shape; attenuating keeps the transient intact and what a listener hears in a bang is its peak, not its 400 ms average. A peak-capped clip is reported as such and exempted from the LUFS-within-1 check.
- WORKFLOW Step 5.2b and `generate-voiceover` SKILL.md: the last four or five narration lines are voiced on their own and listened to before the full run, the same gate segment 1 already has. Why: the ending carries the format's Peak-End points but is heard for the first time only at the watch-through, when a fix costs a re-render; the operator flagged the Krays' sign-off as sounding odd and could not tell whether it was the delivery or the writing. The check also asks whether the close lands on the right beat — a factual coda after a reflective last line deflates it.
- `prompt-hardening-rules.md` R22 and `validate-scenes` malformed check: count the arms on every foreground figure; a figure told to hold something with both hands beside an open window or door grows a third. Why: the Krays' scene 069 shipped with a three-armed driver — both hands on the wheel plus an arm on the sill — and the operator found it on the watch-through after the master was rendered. Three QC groups and two of my own passes over that image missed it, because a third arm at low contrast reads as part of the car.
- `make-thumbnail`: `thumb.py --card` scales the tilted card (sliding it right as it grows so it does not eat the text), and SKILL.md step 1 now says a thumbnail line must land cold on a viewer with no context, and distinguishes a checkable claim from a characterisation. Why: the operator rejected a first pair that distilled the script's conclusions — "NOBODY SAYS THE NUMBER", "THE TOP RUNG 30 YEARS" — as meaningless to a scroller, and ruled that an opinion like "London's most dangerous gang" is fair on a title or thumbnail even though it is forbidden in narration, because their job is the click.
- `make-thumbnail`: `thumb.py --card-ar` sets the card aspect ratio (default unchanged at 1.23). Why: the operator found the script clipped stills badly, and the cause is structural — the card is 1.23 while every still is 1.79, so filling it discards about a third of the width and cuts figures at the sides; no `--centre` value recovers that. A wide card keeps the composition whole at the cost of smaller faces.
- `make-thumbnail`: `thumb.py --import` sizes a thumbnail made elsewhere (exact 16:9 crop, 1280x720, JPEG stepped under YouTube's 2 MB cap, size-check sheet), and SKILL.md now says art direction belongs to the operator and an image model, not to this script. Why: the operator makes better thumbnails in an image model than the compositor can, and does not want Python scripts extended with texture and font options to chase it; what the pipeline should own is the mechanical part — both of his Krays candidates arrived at ~2.5 MB, over YouTube's cap, and one was off-16:9 so a naive resize would have stretched the type.
- `make-thumbnail`: `thumb.py` gains `--font`, `--ink-wear` (worn screen-print lettering), `--card-wear` (hairline scratches and cracks over the photo, eroded card edge), `--tilt`, `--card-x` and `--distress` (aged cream ground). Why: the operator preferred a thumbnail made in an image model and asked whether the look could be reproduced in the pipeline; it can, and the controls make it repeatable per video instead of a two-round conversation with a model. Three implementation notes worth keeping: erode the text's ALPHA, not its colour, or the ink goes pale instead of showing the paper through; generate the wear noise coarse and upscale it, and take the cutoff from the histogram so a fixed fraction of ink lifts (a fixed grey level removes half the letter once blurring narrows the range); and rough a card edge by eroding it with banded noise, because punching circles reads as perforation.
